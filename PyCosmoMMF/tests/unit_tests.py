...
def test_smooth_gauss():
    ...
    kx, ky, kz = PyCosmoMMF.wavevectors3D(dims, box_size)
    f = test_field
    f_smooth = PyCosmoMMF.smooth_gauss(f, R_S, (kx, ky, kz))
    assert f_smooth.shape == (32, 32, 32) #correct shape
    assert np.any(f_smooth != f) #not equal to original
...
def test_smooth_loggauss():
    ...
    kx, ky, kz = PyCosmoMMF.wavevectors3D(dims, box_size)
    f = test_field
    f_smooth = PyCosmoMMF.smooth_loggauss(f, R_S, (kx, ky, kz))
    assert f_smooth.shape == (32, 32, 32) #correct shape
    assert np.any(f_smooth != f) #not equal to original
...
def test_fast_hessian_from_smoothed():
    ...
    f = test_field
    f_smooth = PyCosmoMMF.smooth_gauss(f, R_S, (kx, ky, kz))
    hessian = PyCosmoMMF.fast_hessian_from_smoothed(f_smooth, R_S, (kx, ky, kz))
...
def test_signatures_from_hessian():
    ...
    f_smooth = PyCosmoMMF.smooth_gauss(f, R_S, (kx, ky, kz))
    hessian = PyCosmoMMF.fast_hessian_from_smoothed(f_smooth, R_S, (kx, ky, kz))
    sigs = PyCosmoMMF.signatures_from_hessian(hessian)
    assert sigs.shape == (32, 32, 32, 3)

def test_shrink():
    data = test_field
    new_size = 16
    shrunk = PyCosmoMMF.shrink(data, new_size)
