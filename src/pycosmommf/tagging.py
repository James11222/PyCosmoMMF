...
def make_the_clusbool(delta, max_sigs, verbose, Δ):
    ...
    for i, S_th in enumerate(signature_thresholds):
        ...
        clumps = skimage.measure.label(_clusbool) #an array where 0 is background and 1, 2, 3, ... are the clumps
        total_clumps_detected = np.max(clumps)

        num_clumps_virialized = np.sum(list(map(
                                    functools.partial(check_virialized, delta=delta, clumps=clumps, Δ=Δ), 
...
def calc_structure_bools(delta, max_sigs, verbose, clusbool=None, Smin=-3, Smax=2, Δ=370):
    """
    ...
    if clusbool is None:
        if verbose:
            clusbool, cluster_thresh, S_clus, f_vir_clus = make_the_clusbool(delta, max_sigs, verbose, Δ)
        else:
